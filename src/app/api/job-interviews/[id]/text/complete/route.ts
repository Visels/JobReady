...
import {
  ...
  jobInterviewTextCompleteRequestSchema,
...
export async function POST(
  ...
  try {
    ...
    const input = jobInterviewTextCompleteRequestSchema.parse(
      await requestData(request),
    );
    const state = await service.completeSession(user.id, id, input);
