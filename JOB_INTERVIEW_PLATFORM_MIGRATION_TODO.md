...
- [x] Task 14 - Deterministic question selection
- [x] Task 15 - Behavioral and STAR evaluation
- [x] Task 16 - Role-specific evaluation frameworks
- [x] Task 17 - Interview onboarding
- [x] Task 18 - Text interview vertical slices
- [x] Task 19 - Reports and PDF export
- [x] Task 20 - Realtime voice interviews
- [x] Task 21 - Candidate workspace, dashboard, and navigation

### Commercial and Operations

- [x] Task 22 - Pricing, payments, and cost measurement
- [x] Task 23 - Admin and content operations
- [x] Task 24 - Kenyan launch content and jobs
- [x] Task 25 - Jobready landing page, public brand, and SEO
...
| Task 14 - Deterministic Question Selection | 2026-07-27 | `npx tsc --noEmit`, `npm run lint`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:question-selection`, Task 13 regression `npm run test:job-interviews`, and `git diff --check` passed; focused tests proved expected Scenario A/B slugs, company/industry/role/general fallback levels, target/CV signal weighting, deterministic repeatability, gapless persisted sequences, duplicate prevention, and unanswered/unevaluated persisted turns. | Added deterministic target/CV signal weighting and duplicate prevention to the interview composer, persisted selected questions as `InterviewTurn` rows during job-interview session creation, exposed safe question-set response metadata, added `npm run test:question-selection`, and added `docs/migration/TASK_14_DETERMINISTIC_QUESTION_SELECTION.md`. | No schema or production database migration was required because `InterviewTurn` already stores canonical question-set context; later delivery/evaluation/report tasks should consume persisted turns instead of recomposing; next task is Task 15. |
| Task 15 - Behavioral and STAR Evaluation | 2026-07-27 | `npx tsc --noEmit`, `npm run lint`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:behavioral-evaluation`, Task 14 regression `npm run test:question-selection`, Task 13 regression `npm run test:job-interviews`, and `git diff --check` passed; focused tests proved STAR statuses/scores, independent attached competency scores, required evidence excerpts, non-answer/adversarial/team-only/vague-action/missing-result handling, current-answer-only scoring, no CV/profile-only credit, incompatible-framework rejection, and no invented facts in improved answers. | Added deterministic behavioral STAR evaluator, exported it from interview services, added DB-backed behavioral evaluation fixtures, added `npm run test:behavioral-evaluation`, and added `docs/migration/TASK_15_BEHAVIORAL_STAR_EVALUATION.md`. | No schema or production database migration was required because existing turn/report/STAR/competency score models cover this task; final aggregate report design should revisit report-scoped competency scores; next task is Task 16. |
| Task 16 - Role-Specific Evaluation Frameworks | 2026-07-27 | `npx prisma generate`, `npx tsc --noEmit`, `npm run lint`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:role-specific-evaluation`, `npm run test:interview-content`, Task 14 regression `npm run test:question-selection`, Task 13 regression `npm run test:job-interviews`, Task 15 regression `npm run test:behavioral-evaluation`, and `git diff --check` passed; focused tests proved no STAR fields in technical output, framework-specific Zod schemas, lower scores for incorrect misconceptions than concise correct answers, product/case reward for framing and trade-offs over keywords, coding criteria without execution, evaluator mismatch rejection, and Scenario B technical plus Scenario A role-specific module evaluation. | Added additive follow-up-intent enum migration, role-specific evaluator, Product Manager role-specific template and reviewed plan, load-balancer system-design fixture, Product Manager case-study fixture, DB-backed role-specific evaluation tests, and `docs/migration/TASK_16_ROLE_SPECIFIC_EVALUATION.md`. | Production database was not changed; the enum migration needs normal production deployment review; final report aggregation across multiple `TechnicalScore` turns remains Task 19; next task is Task 17. |
| Task 17 - Interview Onboarding | 2026-07-27 | `npx tsc --noEmit`, `npm run lint`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:interview-onboarding`, `npm run test:interview-content`, Task 13 regression `npm run test:job-interviews`, Task 14 regression `npm run test:question-selection`, `npm run build`, and `git diff --check` passed; focused tests proved Scenario A standalone setup, Scenario B public-job setup with CV opt-in, verified public prefill, technical-label gating, Other Company reviewed role fallback, and keyboard-native controls. | Added onboarding contracts/options/client, authenticated `/interviews/new` and `/interviews/[id]/prepare` routes, public/private target prefill, Other Company fallback messaging, optional CV context disclosure, neutral reviewed fallback plans, practice-link/callback updates, `npm run test:interview-onboarding`, and `docs/migration/TASK_17_INTERVIEW_ONBOARDING.md`. | No schema migration or production database write was performed; the configured Supabase host was rejected by the local-only DB test guard; text delivery remains Task 18, reports remain Task 19, voice remains Task 20, and workspace navigation remains Task 21. |
| Task 18 - Text Interview Vertical Slices | 2026-07-27 | `npx tsc --noEmit`, `npm run lint`, `npm test`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:job-interview-text`, Task 13 regression `npm run test:job-interviews`, Task 14 regression `npm run test:question-selection`, Task 15 regression `npm run test:behavioral-evaluation`, Task 16 regression `npm run test:role-specific-evaluation`, Task 17 regression `npm run test:interview-onboarding`, `npm run test:interview-content`, `npm run build`, and `git diff --check` passed; focused tests proved standalone Scenario A completion, Scenario B behavioral and technical completion, CV opt-in and Skip CV, exact turn order, idempotent retry, recoverable evaluation failure, interruption/resume, final non-answer handling, ownership, and single credit consumption. | Added text-session contracts/service, deterministic general evaluator, aggregate Task 18 evidence rebuilding, authenticated `/api/job-interviews/[id]/text` routes, `/interviews/[id]/room`, accessible text room UI, preparation start CTA, DB-backed `npm run test:job-interview-text`, and `docs/migration/TASK_18_TEXT_INTERVIEW_VERTICAL_SLICES.md`. | No schema migration or production database write was performed; controlled follow-ups are attached to answered persisted turns rather than inserted as new generated turns; final web/PDF report design remains Task 19, realtime voice remains Task 20, and workspace navigation remains Task 21. |
| Task 19 - Reports and PDF Export | 2026-07-28 | `npx tsc --noEmit`, `npm run lint`, `npm test`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:job-interview-reports`, Task 18 regression `npm run test:job-interview-text`, Task 13 regression `npm run test:job-interviews`, Task 14 regression `npm run test:question-selection`, Task 15 regression `npm run test:behavioral-evaluation`, Task 16 regression `npm run test:role-specific-evaluation`, Task 17 regression `npm run test:interview-onboarding`, `npm run test:interview-content`, `npm run build`, Poppler `pdfinfo`, Poppler `pdftoppm` render checks, and `git diff --check` passed; focused tests proved Scenario A/B web/PDF report generation, evidence-backed material claims, incomplete-evidence score suppression, STAR/criteria framework isolation, retry idempotency, web/PDF parity, and long-content page breaks. | Added Task 19 report contracts/service/PDF builder, authenticated `/api/job-interviews/[id]/report` and `/api/job-interviews/[id]/report/pdf`, private `/interviews/[id]/report` page/loading UI, completed-room report links, DB-backed `npm run test:job-interview-reports`, and `docs/migration/TASK_19_REPORTS_AND_PDF_EXPORT.md`. | No schema migration or production database write was performed; mixed rubric versions produce coaching but no readiness score; voice reports should reuse this evidence snapshot in Task 20, and workspace navigation remains Task 21. |
| Task 20 - Realtime Voice Interviews | 2026-07-28 | `npx tsc --noEmit`, `npm run lint`, `npm test`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:job-interview-voice`, Task 18 regression `npm run test:job-interview-text`, Task 19 regression `npm run test:job-interview-reports`, Task 13 regression `npm run test:job-interviews`, Task 14 regression `npm run test:question-selection`, Task 15 regression `npm run test:behavioral-evaluation`, Task 16 regression `npm run test:role-specific-evaluation`, Task 17 regression `npm run test:interview-onboarding`, `npm run test:interview-content`, `npm run build`, and `git diff --check` passed; focused tests proved Scenario A/B voice completion, reconnect without duplicate transcript turns, stable ordered transcript persistence, duplicate-event idempotency/conflict handling, missing-transcript rejection, interrupted-session non-consumption, unauthorized-tool rejection, duration recording, provider usage rows, no video request, D11 raw-audio retention, and Task 19 report evidence parity. | Added voice-session contracts/service, authenticated `/api/job-interviews/[id]/voice` state/connect/event/interrupt/transcript routes, private `/interviews/[id]/voice` room/loading UI, preparation/text-room voice handoffs, reusable persisted-turn answer evaluation seam, configurable realtime deployment/voice handling, DB-backed `npm run test:job-interview-voice`, and `docs/migration/TASK_20_REALTIME_VOICE_INTERVIEWS.md`. | No schema migration or production database write was performed because existing realtime/transcript/report/model-usage tables cover Task 20; realtime deployment and transcription model names remain environment-configured rather than hard-coded; actual production WebRTC depends on a valid Azure OpenAI realtime deployment configured in `AZURE_OPENAI_REALTIME_DEPLOYMENT` or endpoint `?model=`; workspace navigation remains Task 21. |
| Task 21 - Candidate Workspace, Dashboard, and Navigation | 2026-07-28 | `npx tsc --noEmit`, `npm run lint`, `npm test`, `npx prisma validate`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:public-jobs`, `npm run test:applications`, `npm run test:candidate-workspace`, Task 19 regression `npm run test:job-interview-reports`, Task 20 regression `npm run test:job-interview-voice`, `npm run build`, and `git diff --check` passed; focused tests proved Scenario F first sign-in, Scenario G returning workspace, user-scoped dashboard data, expired saved-job history, resumable Scenario D handoff, rubric-safe report trends, and desktop/mobile navigation accessibility contracts. | Replaced the private app shell with a persisted collapsible Jobready side panel, mobile bottom nav, top workspace bar, notifications, account sheet, and focus rail; replaced the visa dashboard with first-login and returning-candidate dashboards; added private `/find-jobs`, `/saved-jobs`, `/applications`, `/cv-resume`, `/reports`, `/career-resources`, `/billing`, `/help`, `/profile`, and `/privacy-data` surfaces; made public job filtering/pagination reusable for `/find-jobs`; added DB-backed `npm run test:candidate-workspace`; and added `docs/migration/TASK_21_CANDIDATE_WORKSPACE_DASHBOARD_NAVIGATION.md`. | No schema migration or production database write was performed; Task 21 exposes honest workspace/list surfaces over existing service layers, while full browser CV/resume tailoring/upload UI, persisted profile preferences, and deletion/export automation remain follow-ups for later commercial/privacy hardening tasks. |
| Task 22 - Pricing, Payments, and Cost Measurement | 2026-07-28 | `npx prisma validate`, `npx prisma generate`, `npx tsc --noEmit`, `npm run lint`, `npm test`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:pricing-payments-costs`, `npm run test:ledger`, `npm run test:tailoring`, `npm run test:job-interviews`, `npm run test:job-interview-text`, `npm run test:job-interview-reports`, `npm run test:job-interview-voice`, `npm run build`, and `git diff --check` passed; focused tests proved Flutterwave replay cannot duplicate grants, failed payments can recover to fulfilled, failed preparation releases reserved tailoring credit, localized pricing and finite entitlements are queryable, and p50/p95 cost distributions work. | Added additive pricing/payment/cost migration, normalized `PricingPlanEntitlement`, Jobready finite product catalog and seed, checkout entitlement snapshots, idempotent purchase-to-ledger fulfillment, failed/refunded purchase states, support references, private billing reconciliation UI, model/storage usage attribution, configurable budget/duration helpers, DB-backed `npm run test:pricing-payments-costs`, and `docs/migration/TASK_22_PRICING_PAYMENTS_COST_MEASUREMENT.md`. | Production DB was updated after local validation: Prisma deploy timed out against the Supabase pooler, so pending additive migrations `20260727120000_add_role_specific_follow_up_intents` and `20260728100000_task22_pricing_payments_costs` were applied with `psql` and recorded in `_prisma_migrations`; pricing seed then upserted 9 plans, 58 price rows, and 9 entitlement rows. Live payment keys were not enabled; Stripe remains fallback and viewing/applying for jobs remains free. |
| Task 23 - Admin and Content Operations | 2026-07-28 | `npx prisma format`, `npx prisma generate`, `npx prisma validate`, `npx tsc --noEmit`, `npm run lint`, `npm test`, clean disposable PostgreSQL `prisma migrate deploy`, `npm run test:admin-content`, `npm run test:jobs`, `npm run test:public-jobs`, `npm run test:interview-content`, `npm run test:job-interviews`, `npm run test:job-interview-reports`, and `npm run build` passed; focused tests proved least-privilege authorization, sourced company-specific question enforcement, candidate wording and rubric preview, retire-not-delete historical safety, JSON/CSV dry-run imports, queues, coverage, job actions, and audit receipts. | Added additive `AdminAuditEvent` migration, admin allowlist authorization, server-side admin operations service, protected `/admin` page, `/api/admin/content` action route, admin-only workspace navigation, validated import preview/apply hooks, job lifecycle wrappers, coverage/queue reports, DB-backed `npm run test:admin-content`, and `docs/migration/TASK_23_ADMIN_CONTENT_OPERATIONS.md`. | No production database write was performed for Task 23; the additive migration is ready for owner-approved deployment, and admin access fails closed until `JOBREADY_ADMIN_*` allowlists are configured. |
| Task 24 - Kenyan Launch Content and Jobs | 2026-07-28 | `npx prisma validate`, `npx tsc --noEmit`, `npm run seed:kenyan-launch-catalog`, `npm run test:kenyan-launch-catalog`, `npm run lint`, `npm test`, `npm run build`, and `git diff --check` passed; focused validation composed Safaricom Software Engineering, Safaricom Product Management, KCB Customer Service, KCB Relationship Management, Kenya Pipeline Graduate Engineering, Kenya Pipeline Engineering, and an unsupported Safaricom Relationship Management fallback with explicit generic-plan warnings. | Added `prisma/jobready-launch-catalog.ts`, direct seed and validation scripts, 11 reviewed official sources, 3 reviewed launch companies, 34 reviewed practice questions, 3 reviewed custom rubrics, 12 published plans, generic Kenya fallback plans, stricter role-family question selection, fallback warnings, and `docs/migration/TASK_24_KENYAN_LAUNCH_CONTENT_AND_JOBS.md`. | The configured development database was updated directly: `npx prisma migrate deploy` timed out against the Supabase pooler, so the pending additive Task 23 migration was applied with `psql -f` and recorded in `_prisma_migrations`; Task 24 seed then upserted the launch catalog. No Task 24 live jobs were published because reviewed KCB role pages were past their 24 July 2026 deadline on the 28 July 2026 review date, and KPC visible vacancies did not match the launch engineering scope with enough closing-date evidence. |
| Task 25 - Jobready Landing Page, Public Brand, and SEO | 2026-07-28 | `npm test`, `npx tsc --noEmit`, `npm run lint`, `npm run build`, `npx prisma validate`, and `git diff --check` passed; build retained the pre-existing edge-runtime static-generation warning, Prisma retained the pre-existing Prisma 7 config deprecation warning, and `git diff --check` reported Windows CRLF normalization warnings only. `npm run test:public-jobs` was attempted but safely blocked because the configured database host is not localhost and the script creates synthetic records without cleanup. | Rebuilt the public landing page, responsive public nav, jobs SEO/filter metadata, sitemap, robots, Open Graph, structured data, auth, referrals, welcome email, legal pages, report PDF labels, error/404 pages, analytics event forwarding, canonical/support defaults, legacy redirects, and `docs/migration/TASK_25_JOBREADY_LANDING_PUBLIC_BRAND_SEO.md`; removed unused legacy marketing/sidebar/promo components. | No schema migration or database write was required; D01/D02/D17 are represented in centralized Jiandae defaults using `jiandae.africa`, `support@jiandae.africa`, and the approved Jiandae asset pack. Legacy visa internals, prompts, historical billing labels, and crawler disallow strings remain documented for Task 30 retirement. |
